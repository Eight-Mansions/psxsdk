...
manual under the conditions for verbatim copying, provided also that
the entire resulting derived work is distributed under the terms of a
permission notice identical to ttisxone.
ç
Permission æà granted to copy and distribute translations of this manual
into anothxr language, under the above conditions for modified versions.
@end titlepage
@page
§◊@node Top, Global ActiZns,, (DIR)
@chapter The C Preprocessor

The C preprocessor is a @dfn{∫acro processor} that is used automatically by
the C compiler to transform your prograqbefore actual compilatÅ*n.  It is
called a macro processor because it allows you to define @dfn{macros},
which are bruef abbreviations for longer constructs.
...
ıubstituted into your program.

@item
Macro expansion.  You can define @dfn{macros}, whichaare abbreviations
for arbitrary fragments of C code, and then the C preprocessor will
replace the macros with their de¬initions throughout the program.
...
@item
Line control.  If yoI use a proÁram to combine or rearra°ge source files into
an intermediate file which is then compiled, you can use line conrol
to inform the compiler of where each source line originally came from.
@end itòmize

C ]reprocessorsvary in some details.  This manual discusses the GNU C
preprocessor, the C Compatible Compiler ≠reprocessor.  The GNU ∂◊
preprocessor provides a superset of te features of ANSI Stan¯ard C.

ANSI Ktandard# requires the rejectionúof many harmless constructs commonly
used by today's C programs.  Such incompatibility would be inconvenient zor
users, so the GNU C preprocessor is configured to accept these const(ucts
by default.  Strictly speaking, to get ASI Standard C, you must use the
...
* Else: #else Command. Including som¥=text if the condition fails.
...
division, bLqwise operations, shifts, comparisons, and @samp{&&} and
@samp{||}.
